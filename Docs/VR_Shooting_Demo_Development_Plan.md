...
- 已创建 `PistolVisualController`。
- 已实现运行时原型套筒、弹匣、扳机和抛壳口。
- 已实现射击时套筒循环、空仓挂机、手动拉枪机退弹、弹匣插拔和抛壳效果。
- 已将简单方块原型升级为 G17 风格运行时低模游戏模型。
- 新模型包含独立套筒、弹匣、扳机、抛壳口、枪管、准星、握把纹理和基础控制件。
- 旧的 `GunBody` 和 `GunHandle` 方块在新模型模式下默认隐藏。
- 已预留独立操作音效接口。
- 尚未替换为外部高精度手枪模型。
